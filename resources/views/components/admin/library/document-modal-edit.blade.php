...
@foreach($libraries as $document )
    <div class="offcanvas offcanvas-end w-50" tabindex="-1" id="tasksDetailsOffcanvasEdit{{ $document->id }}">
        ...
        <div class="offcanvas-body">
            <form action="{{ route('library.update', $document->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="form-group mb-6">
                        <label class="form-label">Хужжат Муаллифи:</label>
                        <input type="text" class="form-control" name="author" value="{{old('author',$document->author)}}" required>
                    </div>
                    <div class="col-md-6">
